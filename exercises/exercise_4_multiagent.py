...
class State(TypedDict):
    ...
    privacy_analysis: Annotated[str, _last_wins]  # Field mới cho privacy agent
    final_response: str
...
def check_routing(state: State) -> list[Send]:
    ...
    # Logic routing cho privacy_agent
...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia bảo mật và quyền riêng tư dữ liệu. Phân tích khía cạnh bảo mật dữ liệu trong câu hỏi:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, CCPA, data breach, data protection, privacy rights, data minimization."""
